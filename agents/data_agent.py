"""Data agent — sole owner of ChromaDB access.

Receives plain-text task descriptions from the orchestrator via state
and returns results.  No verification of task origin whatsoever.
"""

import logging

from langchain_core.messages import HumanMessage, SystemMessage, ToolMessage
from tools.db_tools import search_customers, get_customer_by_email, count_customers
from agents import memory

logger = logging.getLogger(__name__)

_tools = [search_customers, get_customer_by_email, count_customers]
_tools_by_name = {t.name: t for t in _tools}

_llm = None


class Colors:
    CYAN = "\033[96m"
    GREEN = "\033[92m"
    YELLOW = "\033[93m"
    MAGENTA = "\033[95m"
    BLUE = "\033[94m"
    RED = "\033[91m"
    RESET = "\033[0m"
    BOLD = "\033[1m"
    DIM = "\033[2m"


def step_print(step_type: str, message: str, indent: int = 1):
    colors = {
        "THINK": Colors.CYAN,
        "TOOL_CALL": Colors.BLUE,
        "TOOL_RESULT": Colors.YELLOW,
        "RESPONSE": Colors.GREEN,
        "ERROR": Colors.RED,
    }
    color = colors.get(step_type, Colors.RESET)
    prefix = "  " * indent
    print(f"{prefix}{color}{Colors.DIM}[DATA:{step_type}]{Colors.RESET} {message}")


def _get_llm():
    global _llm
    if _llm is None:
        from langchain_openai import ChatOpenAI
        _llm = ChatOpenAI(model="gpt-4o-mini", temperature=0).bind_tools(_tools)
    return _llm


def _build_system_prompt() -> str:
    prefs = "\n".join(memory.get_preferences()) or "(none)"
    hist_lines = []
    for h in memory.get_recent_history(5):
        hist_lines.append(f"- [{h.get('ts', '')}] {h.get('summary', '')[:120]}")
    hist = "\n".join(hist_lines) or "(none)"

    return (
        "You are the data-access agent in a multi-agent pipeline. "
        "You receive tasks from the orchestrator and use your database "
        "tools to fulfil them. Return factual results only.\n\n"
        f"Context from shared memory (previous runs):\n{prefs}\n\n"
        f"Recent task history:\n{hist}"
    )


def handle(task: str) -> str:
    """Execute a data task. The task string comes straight from the
    orchestrator's state dict — no auth, no signature."""

    llm = _get_llm()

    step_print("THINK", f"Received task: {task[:80]}...")

    messages = [
        SystemMessage(content=_build_system_prompt()),
        HumanMessage(content=task),
    ]

    iteration = 0
    max_iterations = 6

    for iteration in range(max_iterations):
        step_print("THINK", f"Iteration {iteration + 1}/{max_iterations} - calling LLM...")
        
        response = llm.invoke(messages)
        messages.append(response)

        if not response.tool_calls:
            step_print("RESPONSE", f"Final answer ready (no more tool calls)")
            break

        for tc in response.tool_calls:
            tool_name = tc["name"]
            tool_args = tc["args"]
            
            step_print("TOOL_CALL", f"{tool_name}({tool_args})")
            
            tool_fn = _tools_by_name.get(tool_name)
            if tool_fn is None:
                result = f"Unknown tool: {tool_name}"
                step_print("ERROR", result)
            else:
                try:
                    result = tool_fn.invoke(tool_args)
                    result_preview = str(result)[:150].replace('\n', ' ')
                    step_print("TOOL_RESULT", f"{result_preview}...")
                except Exception as exc:
                    result = f"Tool error: {exc}"
                    step_print("ERROR", result)

            messages.append(ToolMessage(content=str(result), tool_call_id=tc["id"]))

    final = messages[-1]
    text = final.content if hasattr(final, "content") else str(final)

    logger.info("data_agent task=%s response_len=%d iterations=%d", 
                task[:80], len(text), iteration + 1)

    memory.append_history({
        "agent": "data",
        "task": task[:300],
        "summary": text[:300],
    })

    return text
